...
from nnunetv2.evaluation.evaluate_predictions_2 import compute_metrics_on_folder
...
class nnUNetTrainer(object):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
                 device: torch.device = torch.device('cuda'), network_name='nnUNet'):
        ...
        self.output_folder_base = join(nnUNet_results, self.plans_manager.dataset_name,
                                       self.__class__.__name__ + '__' + self.plans_manager.plans_name + "__" + configuration + "__" + self.network_name_folder) \
    ...
    def print_plans(self):
        if self.local_rank == 0:
            ...
            self.print_to_log_file('initial_lr:', self.initial_lr, '\n', add_timestamp=False)
            self.print_to_log_file('weight_decay:', self.weight_decay, '\n', add_timestamp=False)
            self.print_to_log_file('num_iterations_per_epoch:', self.num_iterations_per_epoch, '\n', add_timestamp=False)
            self.print_to_log_file('num_val_iterations_per_epoch:', self.num_val_iterations_per_epoch, '\n', add_timestamp=False)
            self.print_to_log_file('num_epochs:', self.num_epochs, '\n', add_timestamp=False)

    def configure_optimizers(self):#Mark:
    ...
    def validation_step(self, batch: dict) -> dict:
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            output = self.network(data)
            del data
            l = self.loss(output, target)#Mark:
        ...
        tp, fp, fn, _ = get_tp_fp_fn_tn(predicted_segmentation_onehot, target, axes=axes, mask=mask)#Mark:
    ...
    def on_validation_epoch_end(self, val_outputs: List[dict]):
        outputs_collated = collate_outputs(val_outputs)#Mark:
        ...
        if self.is_ddp:
            ...
            fp = np.vstack([i[None] for i in fps]).sum(0)#Mark:
        ...
        self.logger.log('fn', fn, self.current_epoch)
        self.logger.log('fp', fp, self.current_epoch)
        self.logger.log('tp', tp, self.current_epoch)
    ...
    def on_epoch_end(self):
        ...
        self.print_to_log_file('fn', np.round(self.logger.my_fantastic_logging['fn'][-1], decimals=4))
        self.print_to_log_file('fp', np.round(self.logger.my_fantastic_logging['fp'][-1], decimals=4))
        self.print_to_log_file('tp', np.round(self.logger.my_fantastic_logging['tp'][-1], decimals=4))
