...
from MBUNeXt.paths import nnUNet_preprocessed
from MBUNeXt.training.Trainer.nnUNetTrainer import nnUNetTrainer
from MBUNeXt.training.Trainer.MBUNeXtTrainer import MBUNeXtTrainer
from MBUNeXt.utilities.dataset_name_id_conversion import maybe_convert_to_dataset_name
from MBUNeXt.utilities.find_class_by_name import recursive_find_python_class
...
def get_trainer_from_args(dataset_name_or_id: Union[int, str],
                          ...
                          network_name: str = 'nnUNet'):
    # load nnunet class and do sanity checks
    nnunet_trainer = recursive_find_python_class(join(MBUNeXt.__path__[0], "training", "Trainer"),
                                                trainer_name, 'MBUNeXt.training.Trainer')
    if nnunet_trainer is None:
        raise RuntimeError(f'Could not find requested nnunet trainer {trainer_name} in '
                           f'MBUNeXt.training.Trainer ('
                           f'{join(MBUNeXt.__path__[0], "training", "Trainer")}). If it is located somewhere '
    ...
    nnunet_trainer = nnunet_trainer(plans=plans, configuration=configuration, fold=fold, dataset_json=dataset_json,
                                    unpack_dataset=not use_compressed, device=device, network_name=network_name)
...
def run_ddp(rank, dataset_name_or_id, configuration, fold, tr, p, use_compressed, disable_checkpointing, c, val,
            pretrained_weights, npz, val_with_best, world_size, network_name):
    setup_ddp(rank, world_size)
    torch.cuda.set_device(torch.device('cuda', dist.get_rank()))

    nnunet_trainer = get_trainer_from_args(dataset_name_or_id, configuration, fold, tr, p,
                                           use_compressed, network_name=network_name)
...
def run_training(dataset_name_or_id: Union[str, int],
    ...
    if num_gpus > 1:
        ...
        nnunet_trainer = get_trainer_from_args(dataset_name_or_id, configuration, fold, trainer_class_name,
                                               plans_identifier, use_compressed_data, device=device, network_name=network_name)
...
def run_training_entry():
    ...
    parser.add_argument('-dataset_name_or_id', type=str, default='139',
                        help="Dataset name or ID to train with")
    parser.add_argument('-configuration', type=str, default='3d_fullres_128_b2',
                    help="Configuration that should be trained")
    parser.add_argument('-fold', type=str, default='0',
                        help='Fold of the 5-fold cross-validation. Should be an int between 0 and 4.')
    parser.add_argument('-tr', type=str, required=False, default='MBUNeXtTrainer',
                        help='[OPTIONAL] Use this flag to specify a custom trainer. Default: nnUNetTrainer')
    parser.add_argument('-net', type=str, required=False, default='MBUNeXt',
                        help='[OPTIONAL] Use this network name to make a new training folder. Default: MBUNeXt')
    parser.add_argument('-p', type=str, required=False, default='nnUNetPlans_dd',
                        help='[OPTIONAL] Use this flag to specify a custom plans identifier. Default: nnUNetPlans')
    parser.add_argument('-pretrained_weights', type=str, required=False, default=None,
                        help='[OPTIONAL] path to nnU-Net checkpoint file to be used as pretrained model. Will only '
                             'be used when actually training. Beta. Use with caution.')
    parser.add_argument('-num_gpus', type=int, default=2, required=False,
    ...
    run_training(args.dataset_name_or_id, args.configuration, args.fold, args.tr, args.p, args.pretrained_weights,
                 ...
                 device=device, network_name=args.net)
