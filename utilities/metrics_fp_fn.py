...
def compute_metrics2(gt_array, pred_array, mask_is_empty=False, voxel_vol=(2.03642 * 2.03642 * 3.0) / 1_000):
    if mask_is_empty:
        dice_sc = np.nan
...
def compute_metrics_dice_fnv_fpv(gt_array, pred_array, mask_is_empty=False,
                                 voxel_vol=(2.03642 * 2.03642 * 3.0) / 1_000):
    gt = np.squeeze(gt_array, axis=0)
    pred = np.squeeze(pred_array, axis=0)
    if mask_is_empty:
        dice_sc = np.nan
        false_neg_vol = np.nan
    else:
        dice_sc = dice_score(gt, pred)
        false_neg_vol = false_neg_pix(gt, pred) * voxel_vol

    false_pos_vol = false_pos_pix(gt, pred) * voxel_vol

    return dice_sc, false_neg_vol, false_pos_vol


if __name__ == "__main__":
    # nii_gt_path, nii_pred_path = sys.argv
    nii_gt_path = '/data3/zoe/Code/AutoPET_Challenge_Submission-main/path_gt/PETCT_0f4ee9e078_02-10-2005-NA-PET-CT Ganzkoerper  primaer mit KM-08757.nii.gz'
    nii_pred_path = '/data3/zoe/Code/AutoPET_Challenge_Submission-main/path_pred/PETCT_0f4ee9e078_02-10-2005-NA-PET-CT Ganzkoerper  primaer mit KM-08757.nii.gz'
